...
class LoRA_Sam(nn.Module):
    ...
    def __init__(self, sam_model: Sam, r: int, lora_layer=None):
        ...
        for name, param in sam_model.prompt_encoder.named_parameters():
            if 'no_mask_embed' not in name:
                param.requires_grad = False
        # 冻结mask_decoder.transformer所有参数
    ...
    def save_lora_parameters(self, filename: str) -> None:
        ...
        fa_ti_tensors = {'fati_qa': self.fa_ti_q_proj_A.weight, 
                         'fati_qb': self.fa_ti_q_proj_B.weight,
                         'fati_va': self.fa_ti_v_proj_A.weight,
                         'fati_vb': self.fa_ti_v_proj_B.weight}
        
        dense_prompt = {'no_mask_embed': self.sam.prompt_encoder.no_mask_embed.weight}
        ...
        merged_dict = {**sa_a_tensors, **sa_b_tensors, **cti_a_tensors, **cti_b_tensors,
                       **cit_a_tensors, **cit_b_tensors, **fa_ti_tensors, **dense_prompt, **mask_decoder_tensors}
    ...
    def load_lora_parameters(self, filename: str, device: torch.device) -> None:
        ...
        self.sam.prompt_encoder.no_mask_embed.weight = Parameter(state_dict["no_mask_embed"])
