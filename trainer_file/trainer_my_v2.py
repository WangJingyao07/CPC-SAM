...
def trainer(args, model, snapshot_path, multimask_output, low_res):
    if args.dataset == 'data1':
        from datasets.dataset_celebAMMask_HQ_v2 import Synapse_dataset, RandomGenerator
    else:
        print("##### Unimplemented dataset #####")
        sys.exit()
    
    exp_name = f"{args.dataset}-{args.num_data}-{args.exp_type}-lora_mask"
    logger = wandb.init(project='Auto-sam', name=exp_name, resume='allow', anonymous='must',
    ...
    #     ]),
    # )
    trans = transforms.Compose([RandomGenerator(
                output_size=[args.img_size, args.img_size], low_res=[low_res, low_res])
            ])
    # trans = None
    db_train = Synapse_dataset(train_dir=args.root_path, num_data=args.num_data,
                    transform=trans,
                    train_flag="train")
    # db_valid = Synapse_dataset(train_dir=args.root_path, num_data=args.num_data,
    #                            transform=trans,
    #                            train_flag="valid")
    # db_test = Synapse_dataset(train_dir=args.root_path, num_data=args.num_data,
    #                            transform=trans,
    #                            train_flag="test")
    ...
    train_loader = DataLoader(db_train, batch_size=args.batch_size, num_workers=32, pin_memory=True,
                             worker_init_fn=worker_init_fn, sampler=selector[:num_train])
    valid_loader = DataLoader(db_train, batch_size=args.batch_size, num_workers=1, pin_memory=True,
                             worker_init_fn=worker_init_fn, sampler=selector[num_train:])

    # train_loader = DataLoader(db_train, batch_size=args.batch_size, num_workers=32)
    # valid_loader = DataLoader(db_valid, batch_size=args.batch_size, num_workers=32)
    # test_loader = DataLoader(db_test, batch_size=args.batch_size, num_workers=32)
    # train_loader = DataLoader(db_train, batch_size=args.batch_size, num_workers=32, pin_memory=True)
    # valid_loader = DataLoader(db_valid, batch_size=args.batch_size, num_workers=32, pin_memory=True)
    # test_loader = DataLoader(db_test, batch_size=args.batch_size, num_workers=32, pin_memory=True)
    ...
    for epoch_num in range(max_epoch):
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if iter_num % show_num == 0:
                logging.info('iteration {}/{} : loss : {:.4f}, loss_ce: {:.4f}, '
                             ...
                             (iter_num, len(train_loader), loss.item(),
        ...
        # valid_score = validate(args, model, valid_loader, multimask_output)
