...
root = "/media/wjy/C/dataset/hand2K/GTEA/"
...
mask_img_root = root + "Masks/"
save_root = root + "masks/"
save_root1 = root + "images/"
mask_img_value_list = []
for i in range(len(img_name_list)):
    img_name = img_name_list[i]
    mask_img_path = mask_img_root + img_name.split(".")[0]+".png"
    mask_img = cv2.imread(mask_img_path)
    mask_img = mask_img[:, :, 0]
    mask_img[mask_img == 100] = 1
    mask_img[mask_img == 150] = 2
    if (mask_img > 2).any():
        continue
    np.save(save_root + img_name.split(".")[0]+".npy", mask_img)
    shutil.copyfile(img_root + img_name, save_root1 + img_name)
    # cv2.imread(save_root + img_name.split(".")[0] + ".npy", mask_img)
    # print("")
# p = 0
# img_dict = {}
# with open('/media/wjy/C/dataset/X-ray/archive/jsrt_metadata.csv', mode='r', encoding='utf-8') as file:
...
#         if x == "" or y == "" or size == "":
#             continue
#         img_dict[img_name] = [(int(x), int(y)), (int(x)+int(size), int(y)+int(size))]
# save_root = ""
